...
function App() {
  ...
  const startRandom = (count) => { setQCount(count === 'ALL' ? 12 : count); resetGameState(); setIsStarted(true); setView('game'); };

  // 🌟 クラス・番号・氏名の3項目に変更し、例を削除
  if (!isRegistered) {
    return (
  ...
  return (
    <div style={{ textAlign: 'center', backgroundColor: '#f0f8ff', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {hasData ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {view === 'settings-select' && gameMode === 'random' && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '30px', alignItems: 'center' }}>
              <h2 style={{ fontSize: '2.5rem', color: '#4ECDC4' }}>Select Question Count</h2>
              <div style={{ display: 'flex', gap: '30px' }}>
                <button onClick={() => startRandom(5)} className="count-btn">5問</button>
                <button onClick={() => startRandom(8)} className="count-btn">8問</button>
                <button onClick={() => startRandom('ALL')} className="count-btn">ALL<br/><span style={{fontSize:'1rem'}}>(全12問)</span></button>
              </div>
              <button onClick={() => setView('mode-select')} style={{ marginTop: '20px', background: 'none', border: 'none', color: '#888', cursor: 'pointer', textDecoration: 'underline', fontSize: '1.2rem' }}>← もどる</button>
            </div>
          )}
      ...
      <style>{`
        .mode-card { cursor: pointer; padding: 40px; background-color: #fff; border-radius: 25px; box-shadow: 0 10px 20px rgba(0,0,0,0.1); width: 280px; box-sizing: border-box; transition: transform 0.2s, box-shadow 0.2s; }
